...
function saveSettings() {
    ...
    for (const card of editingCards) {
        ...
        if (!name) { alert("Every card needs a name."); return; }
        const baseMpd = Number(card.baseMpd);
        if (!Number.isFinite(baseMpd) || baseMpd < 0) { alert(`${name}: base mpd must be a number ≥ 0.`); return; }
        const resetDay = Math.round(Number(card.resetDay));
        if (!Number.isFinite(resetDay) || resetDay < 1 || resetDay > 31) { alert(`${name}: reset day must be between 1 and 31.`); return; }
        const minSpendRaw = String(card.minSpend ?? "").trim();
        const minSpend = minSpendRaw === "" ? null : Number(minSpendRaw);
        if (minSpend !== null && (!Number.isFinite(minSpend) || minSpend < 0)) { alert(`${name}: min spend must be a number ≥ 0.`); return; }
        ...
        if (rounding === "block" && (!Number.isFinite(blockSize) || blockSize <= 0)) { alert(`${name}: block size must be a positive number.`); return; }

        if (!card.rules.length) { alert(`${name}: needs at least one earn rule.`); return; }
        const rules = [];
        for (const rule of card.rules) {
            ...
            if (!Number.isFinite(mpd) || mpd < 0) { alert(`${name}: rule mpd must be a number ≥ 0.`); return; }
            if (!Number.isFinite(cap) || cap < 0) { alert(`${name}: rule cap must be a number ≥ 0.`); return; }
            const cats = String(catsText).split(",").map(c => c.trim().toLowerCase()).filter(Boolean);
            if (!cats.length) { alert(`${name}: every rule needs at least one category.`); return; }
            if (cats.includes("shared")) { alert(`${name}: "shared" is not a valid category name.`); return; }
...
window.onload = () => {
    ...
    // Offline support: served relative so it works under a GitHub Pages subpath
    if ("serviceWorker" in navigator) {
        navigator.serviceWorker.register("sw.js").catch(() => { /* offline still degrades to Safari cache */ });
    }
};
